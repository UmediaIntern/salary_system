...
export function BonusPositionTypeBatchCreateForm<
...
>({
	...
	function DynamicForm() {
		const { control, handleSubmit, register } = useForm<FormValues>({
			...
			defaultValues: {
				bonus_position_type: [], // Start with an empty array
			},
		});

		const { fields, append, remove } = useFieldArray({
			control,
			name: "bonus_position_type",
		});

		const onSubmit = async (data: FormValues) => {
			data.bonus_position_type.map(async (x) => {
				console.log(x);
						...
						<div className="flex-grow space-y-4 overflow-y-auto p-4">
								...
								>
									<div className="flex w-full flex-row items-center justify-between gap-4">
										...
										<Input
											...
											type="string"
											{...register(
												`bonus_position_type.${index}.position_type` as const
											)}
										/>
										{/* multiplier Input */}
										<Label className="min-w-[75px] text-center">
										...
										<Input
											...
											type="number"
											{...register(
												`bonus_position_type.${index}.multiplier` as const
											)}
										/>
							...
							<div className="flex justify-center">
								<Button
									type={"button"}
									variant={"ghost"}
									onClick={() => append({ position_type: "", multiplier: 0 })}
								>
									<PlusCircle />
								</Button>
							</div>
