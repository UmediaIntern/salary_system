...
import {
	MonthSalaryStatusEnum,
	type MonthSalaryStatusEnumType,
} from "~/server/api/types/month_salary_status_enum";
...
> {
	...
	async getEmployeeDataWithInfo(
		...
		const EmployeeDataFE = await this.addEmployeeInformation(
			period_id,
			dec
		);
	...
	async addEmployeeInformation(
		...
		const emp_no_list_month_salary = await this.getCurrentMonthPayEmpNo(
			period
		);
		return data.map((e) => {
	...
	async getCurrentMonthPayEmpNo(period: number): Promise<string[]> {
		...
		const employees = await syncService.getCandPaidEmployees(
			FunctionsEnum.enum.month_salary,
			period
		);
