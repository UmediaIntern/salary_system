...
export class OtherMapper {
	...
	async getDifference(
		...
		const [expense_list, previous_expense_list] = await Promise.all([
			this.ehrService.getExpenseByEmpNoList(cur_period_id, emp_no_list),
			this.ehrService.getExpenseByEmpNoList(
				previous_period_id,
				emp_no_list
			),
		...
		const [
			...
			this.employeeDataService.getCurrentEmployeeData(cur_period_id),
			this.employeeDataService.getCurrentEmployeeData(previous_period_id),
			this.employeePaymentService.getCurrentEmployeePayment(cur_period_id),
			this.employeePaymentService.getCurrentEmployeePayment(
				previous_period_id
			),
		]);
		...
		const difference: OtherFEDiffType[] = await Promise.all(
			emp_no_list.map(async (emp_no) => {
				...
				return {
					...
					brokerage_fee: broker !== prev_broker,
					retirement_income: retire !== prev_retire,
					l_i_disability_reduction: l_i_reduction !== prev_l_i_reduction,
					h_i_subsidy: h_i_subsidy !== prev_h_i_subsidy,
				};
			})
		);
