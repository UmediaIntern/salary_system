...
export class TransactionService {
	...
	async createTransaction(
		...
		const employee_payment = commonParameters.employee_payment_list.find(
			(e) => e.emp_no === emp_no
		);
		const employee_trust = commonParameters.employee_trust_list.find(
			(e) => e.emp_no === emp_no
		);
		...
		const accumulated_bonus = commonParameters.accumulated_bonus_list.find(
			(e) => e.emp_no === emp_no
		);
		const accumulated_trust = commonParameters.accumulated_trust_list.find(
			(e) => e.emp_no === emp_no
		);
		...
		const reissue_salary = await this.calculateService.getReissueSalary(
			expense_list,
			expense_class_list
		);
		...
		const shift_allowance = await this.ehrService.getTargetAllowance(
			allowance_list,
			allowance_type_list,
			emp_no,
			"輪班津貼"
		...
		const leave_deduction = await this.calculateService.getLeaveDeduction(
			employee_data!,
			holiday_list!,
			holidays_type_list,
			insurance_rate_setting!,
		...
		const other_deduction = await this.calculateService.getOtherDeduction(
			expense_list,
			expense_class_list
		);
		const other_addition = await this.calculateService.getOtherAddition(
			expense_list,
			allowance_type_list
		);
		const meal_deduction = await this.calculateService.getMealDeduction(
			expense_list,
			expense_class_list
		);
		const taxable_income = await this.calculateService.getTaxableIncome(
			discounted_employee_payment!,
			exceed_overtime_pay,
			professional_cert_allowance
		);
		const income_tax = await this.calculateService.getSalaryIncomeTax(
			employee_data!,
			issue_date,
		...
		const end_of_year_bonus = await this.calculateService.getYearEndBonus(
			bonus_list,
			bonus_type_list
		);
		const taxable_subtotal = await this.calculateService.getTaxableSubtotal(
			pay_type,
			discounted_employee_payment!,
			operational_performance_bonus,
			reissue_salary,
		...
		const l_i_pay = await this.calculateService.getLaborInsurancePay(
			discounted_employee_payment!,
			employee_data!,
			insurance_rate_setting!,
			payset,
		...
		const salary_advance = await this.calculateService.getSalaryAdvance(
			pay_type,
			payset,
			discounted_employee_payment!,
			insurance_rate_setting!,
		...
		const h_i_pay = await this.calculateService.getHealthInsurancePay(
			discounted_employee_payment!,
			employee_data!,
			insurance_rate_setting!
		);
		const group_insurance_pay =
			await this.calculateService.getGroupInsurancePay(employee_data!);
		...
		const brokerage_fee = await this.calculateService.getBrokerageFee(
			expense_list,
			expense_class_list
		);
		...
		const dorm_deduction = await this.calculateService.getMealDeduction(
			expense_list,
			expense_class_list
		); // ~ Need Check
